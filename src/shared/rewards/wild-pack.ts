import { OPEN_EQUIPMENT_LEVELS } from '../engine/combat-v6/equipment/realm';
...
export const WILD_EQUIPMENT_REWARDS = new Map<string, { slot: DaoEquipmentSlot; level: number }>(
  OPEN_EQUIPMENT_LEVELS.flatMap(level => DAO_EQUIPMENT_SLOTS.map(slot => [`equipment.${slot}.${level}`, { slot, level }] as const)),
);
