...
策划编辑本目录四份 JSON。各自的 `$schema` 提供字段提示，`../pack.ts` 负责结构与跨包语义校验；非法内容会停止加载，并报告文件、字段和相关 ID。代码仍负责随机顺序、取整、机制执行与状态操作。

## 1. 物种与个体生成：species.json

当前物种包 `formatVersion: 2`、`contentRevision: 5`，包含炼气至化神十五种物种。讨论中的“结丹”对应游戏实际境界键“金丹”。每条配置包含名称、图标、习性描述、开放境界、携带等级、是否可领取为初始伙伴、五项资质范围、成长范围及出生技能规则。不再提供固定定位或物种加点偏好。
...
| `birthSkills.core` | 出生必带技能，可被兽诀覆盖 |
| `birthSkills.candidates` | 普通候选池，等权、不重复补抽 |
| `birthSkills.extraCountWeights` | 先抽额外数量，`count`为数量、`weight`为百分比整数，合计100 |
| `generation.minBirthSkills` / `maxBirthSkills` | 全包出生技能数量边界，当前2至4 |
| `generation.starterLevel` / `lifespan` | 初始伙伴等级10、初始及最大寿命1000 |

物种目录不再依赖野外怪物目录。保留旧物种ID，未来野怪编排通过ID引用物种并调用同一个抽取函数；新增十二种暂未加入野外遭遇池，不代表已开放捕捉。已有野外怪物仍使用自己的战斗参数。
...
初始领取和捕捉共用该底座，实际技能数即格位容量。旧捕捉附加20%抽签已删除；新个体属性点全部待分配。兽诀仍只替换，不增加技能格。新个体记录生成算法版本`summoned_beast_v2`、物种内容修订`generationContentRevision`与种子。旧v1个体继续按保存的事实解析，不重抽、不重置加点、不要求落在新物种区间内；无数据库迁移。

校验拒绝重复技能、核心／候选重叠、未知技能、高低级同族混入、重复额外数量、权重不合计100、候选不足、出生格数越界、区间倒置、非法初始伙伴及境界等级不一致。规则明细与后续接入边界见[物种与生成底座定稿](../../../../../../docs/combat-v6-beast-species-generation.md)。
