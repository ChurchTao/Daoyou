...
| `birthSkills.core` | 出生必带技能，可被传承灵印覆盖 |
...
初始领取和捕捉共用该底座，实际技能数即格位容量。旧捕捉附加20%抽签已删除；新个体属性点全部待分配。传承灵印仍只替换，不增加技能格。新个体记录生成算法版本`summoned_beast_v2`、物种内容修订`generationContentRevision`与种子。旧v1个体继续按保存的事实解析，不重抽、不重置加点、不要求落在新物种区间内；无数据库迁移。
