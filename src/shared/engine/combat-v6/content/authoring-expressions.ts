  ...
  if (typeof value === 'string' && formula) {
    try {
      for (const name of value.match(/[A-Za-z_][A-Za-z0-9_.]*/g) ?? [])
        if (!allowedVariables.has(name) && !/^(resource\.lingxiao\.resource\.[a-z_]+|uses\.lingxiao\.skill\.[a-z_]+|fact\.(weaponXuanfengLevel|weaponXuanfengAttack|metalFireWindEquipmentCount))$/.test(name)) throw new Error('未知表达式标识：' + name);
