...
import { TianyanSkillsShape, loadTianyanSkills, TIANYAN_SKILLS } from './tianyan-skill-pack';
...
describe('天衍基础技能配置', () => {
  ...
  it('仅七个基础技能；法印在成功施放末尾更新且作用于自身', () => {
    ...
    expect(fire.effects.filter(e => e.type === 'emitMechanic')).toHaveLength(2);
    expect(fire.successEffects).toEqual([{ type: 'applyStatus', statusId: 'tianyan.status.mark.fire', duration: 1, targeting: { side: 'self' } }]);
