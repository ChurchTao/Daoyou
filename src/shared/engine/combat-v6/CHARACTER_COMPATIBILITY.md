...
| `condition.tracks.bodyCultivation` | 等级和等级内进度可迁移 | 五轨等级一对一迁移为四修炼 + 生命根基；旧 modifier、Buff 和肉身阶位战斗效果全部退出 |
...
2. 物理与法术是否共用暴击率；是否保留暴击抵抗和暴伤减免。
3. 是否保留物理/法术穿透，还是完全服从梦幻式攻防公式。
4. 灵根和境界压制是保留、改版，还是退出战斗系统。
5. 治疗以哪个六维为主，以及 `healPower` 的面板公式。
6. 身法如何换算 `hit/dodge` 点数，神识如何换算 `sealHit/sealResist` 点数。
7. 生命根基每级提供多少 `maxHp/healPower`。
8. 召唤兽进入哪个阶段；进入前需要先建立独立持久模型。
