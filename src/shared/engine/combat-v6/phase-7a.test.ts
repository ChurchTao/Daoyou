...
function progress(sectId: CombatV6SectId): SectCombatProgressV6 {
  ...
  return {
    ...
    methods: Object.fromEntries(
      definition.methods.map((method) => [method.id, 180]),
    ),
    meridianDepth: 0,
    activePathId: definition.paths[0].id,
    meridianLoadouts: definition.paths.map((path) => ({
      pathId: path.id,
      nodeIds: [],
      revision: 1,
...
function input(
  encounterId = TRAINING_ENCOUNTER_ID.SingleDummy,
  tier: CombatV6TrainingTierV1 = 60,
  sectId: CombatV6SectId = 'lingxiao',
  seed = 17,
) {
  return {
    encounterId,
...
describe('combat-v6 Phase 7A 内容与编译', () => {
  it('训练内容、三档显式NPC和六类遭遇完整', () => {
    ...
    for (const tier of [60, 120, 180] as const) {
      for (const encounter of COMBAT_V6_TRAINING_ENCOUNTERS_V1) {
        const result = compileCombatV6TrainingEncounterV1(
          input(encounter.id, tier),
        );
        ...
        expect(
          result.compiled.battleInput.units.some(
            (unit) => unit.side === Team.B,
          ),
        ).toBe(true);
        const player = result.compiled.battleInput.units.find(
          (unit) => unit.id === 'player',
        )!;
  ...
  it('未知内容、无效面板和冲突ID稳定阻止编译', () => {
    ...
    const codes = validateCombatV6TrainingContentV1(content).map(
      (item) => item.code,
    );
...
describe('combat-v6 Phase 7A 指令查询', () => {
  it('查询资源、目标和基础指令且不修改状态、事件或RNG', () => {
    ...
    expect(options.skills[0]).toMatchObject({
      skillId: skill.id,
      ready: false,
      reasons: [FailReason.InsufficientMp],
      selectableTargetIds: ['target'],
      targetCount: 1,
    });
...
describe('combat-v6 Phase 7A Host', () => {
  it('要求玩家指令、允许锁定前覆盖并执行确定性NPC策略', () => {
    const result = createCombatV6TrainingHostV1(
      input(TRAINING_ENCOUNTER_ID.SingleSparring),
    );
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(() => result.host.resolveRound()).toThrowError(
      expect.objectContaining({
        code: TrainingHostErrorCode.PlayerCommandMissing,
      }),
    );
    const target = result.host.queryCommands().attackTargetIds[0];
    result.host.submit(result.host.playerId, { type: CommandType.Defend });
    result.host.submit(result.host.playerId, {
    ...
    expect(
      events.some(
        (event) =>
          ...
          event.unitId === result.host.playerId &&
    ...
    expect(
      result.host
        .trace()
        .rounds[0].commands.find(
          (entry) => entry.unitId === result.host.playerId,
        )?.command.type,
    ).toBe(CommandType.Attack);
  ...
  it('只接受玩家的结构合法指令', () => {
    ...
    const npc = result.host.state.units.find(
      (unit) => unit.id !== result.host.playerId,
    )!;
    expect(() =>
      result.host.submit(npc.id, { type: CommandType.Defend }),
    ).toThrowError(
      expect.objectContaining({
        code: TrainingHostErrorCode.UnitNotControlled,
      }),
    );
    expect(() =>
      result.host.submit(result.host.playerId, {
      ...
      expect.objectContaining({ code: TrainingHostErrorCode.UnknownSkill }),
    );
    expect(() =>
      result.host.submit(result.host.playerId, {
    ...
    ).toThrowError(TrainingHostError);
  });

  it('净化和复活条件由正常技能与伤害形成', () => {
    const cleanse = createCombatV6TrainingHostV1(
      input(TRAINING_ENCOUNTER_ID.SupportCleanse, 60, 'wuxiang'),
    );
    const revive = createCombatV6TrainingHostV1(
      input(TRAINING_ENCOUNTER_ID.SupportRevive, 60, 'wuxiang'),
    );
    ...
    const afflicted = cleanse.host.state.units.find(
      (unit) => unit.name === '受术同伴',
    )!;
    expect(
      afflicted.statuses.some(
        (status) => status.id === 'combat.training.status.control',
      ),
    ).toBe(true);
    cleanse.host.submit(cleanse.host.playerId, {
      type: CommandType.Skill,
      skillId: WUXIANG_SKILL_ID.Purify,
      targets: [afflicted.id],
    });
    cleanse.host.resolveRound();
    expect(
      cleanse.host.state.units
        .find((unit) => unit.id === afflicted.id)!
        .statuses.some(
          (status) => status.id === 'combat.training.status.control',
        ),
    ).toBe(false);
    ...
    const fallen = revive.host.state.units.find(
      (unit) => unit.name === '待援同伴',
    )!;
    ...
    expect(
      revive.host
        .trace()
        .events.some(
          (event) => event.type === 'unitDowned' && event.unitId === fallen.id,
        ),
    ).toBe(true);
    revive.host.submit(revive.host.playerId, {
      type: CommandType.Skill,
      skillId: WUXIANG_SKILL_ID.Revive,
      targets: [fallen.id],
    });
    revive.host.resolveRound();
    expect(
      revive.host.state.units.find((unit) => unit.id === fallen.id)!.flags
        .downed,
    ).toBe(false);
  });
  ...
  it('相同输入和指令得到相同不可变调试转录', () => {
    const run = () => {
      const result = createCombatV6TrainingHostV1(
        input(TRAINING_ENCOUNTER_ID.TripleDummy, 60, 'tianyan', 99),
      );
      if (!result.ok) throw new Error('训练Host创建失败');
      result.host.submit(result.host.playerId, { type: CommandType.Defend });
  ...
  it('从Redis运行快照恢复后保持指令、RNG、事件和终态确定性', () => {
    const created = createCombatV6TrainingHostV1(
      input(TRAINING_ENCOUNTER_ID.SingleSparring, 60, 'jiujie', 771),
    );
    if (!created.ok) throw new Error('训练Host创建失败');
    const target = created.host.queryCommands().attackTargetIds[0];
    created.host.submit(created.host.playerId, {
      type: CommandType.Attack,
      target,
    });
    const restored = restoreCombatV6TrainingHostV1(
      created.host.runtimeSnapshot(),
    );
    ...
    expect(
      CombatV6RedisRuntimeV1Schema.safeParse({
        ...
        metadata: {
          ...
          battleType: 'training',
          idempotencyKey: 'c457d5b7-d6be-471b-9dc2-d17bd267e339',
          payload: {
            encounterId: TRAINING_ENCOUNTER_ID.SingleSparring,
            tier: 60,
          },
        },
        revision: 1,
  ...
  it('将内核终局稳定解释为训练结果', () => {
    ...
    expect(
      trainingEncounterOutcome(base, result.host.playerId),
    ).toBeUndefined();
    expect(
      trainingEncounterOutcome(
        { ...base, result: { winner: Team.A, reason: 'wipe' } },
        result.host.playerId,
      ),
    ).toBe('victory');
    expect(
      trainingEncounterOutcome(
        { ...base, result: { winner: Team.B, reason: 'wipe' } },
        result.host.playerId,
      ),
    ).toBe('defeat');
    expect(
      trainingEncounterOutcome(
        { ...base, result: { winner: 'draw', reason: 'round-limit' } },
        result.host.playerId,
      ),
    ).toBe('draw');
    expect(
      trainingEncounterOutcome(
        { ...base, result: { winner: Team.B, reason: 'flee' } },
        result.host.playerId,
      ),
    ).toBe('aborted');
  });
