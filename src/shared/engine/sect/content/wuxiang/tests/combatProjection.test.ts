...
const abilityTable: Record<PathId, Record<string, {
  ...
  'mirror-karma': {
    'flower-heart': { names: ['拈花叩心', '花落问罪', '心花两忘'], target: 'enemy', cost: 0.05, cooldown: 0 },
    'blood-tide': { names: ['血海听潮', '血海回澜', '海月同潮'], target: 'self', cost: 0.08, cooldown: 3 },
    'three-knocks': { names: ['三叩业门', '业门倒叩', '门内无人'], target: 'enemy', cost: 0.07, cooldown: 2 },
    'observe-calamity': { names: ['闭目观劫', '开眼见劫', '劫相俱寂'], target: 'self', cost: 0.10, cooldown: 4 },
    'five-skandhas': { names: ['照见五蕴', '五蕴还照', '五蕴皆空'], target: 'enemy', cost: 0.06, cooldown: 3 },
    'reed-crossing': { names: ['一苇横江', '一苇倒渡', '此岸非岸'], target: 'self', cost: 0.08, cooldown: 5 },
  },
  'demon-crossing': {
    'flower-heart': { names: ['拈花叩心', '摘心问魔', '心魔两忘'], target: 'enemy', cost: 0.06, cooldown: 0 },
    'blood-tide': { names: ['血海听潮', '血海倒悬', '血海无涯'], target: 'self', cost: 0.14, cooldown: 3 },
    'three-knocks': { names: ['三叩业门', '三叩魔关', '业门无生'], target: 'enemy', cost: 0.09, cooldown: 2 },
    'observe-calamity': { names: ['闭目观劫', '开眼见魔', '劫火自明'], target: 'self', cost: 0.11, cooldown: 4 },
    'five-skandhas': { names: ['照见五蕴', '焚尽五蕴', '蕴空身在'], target: 'self', cost: 0.07, cooldown: 3 },
    'reed-crossing': { names: ['一苇横江', '一苇渡厄', '苦海无舟'], target: 'self', cost: 0.10, cooldown: 5 },
  },
};
...
describe('无相禅宗战斗投影', () => {
  ...
  it.each(['mirror-karma', 'demon-crossing'] as const)(
    ...
    (pathId) => {
      ...
      expect(() => new SectStateValidator().validate(WUXIANG_MODULE, sect)).not.toThrow();
      expect(WUXIANG_MODULE.definition.configVersion).toBe(2);
    },
  );
  ...
  it.each(['mirror-karma', 'demon-crossing'] as const)(
    ...
    (pathId) => {
      ...
      for (const abilityId of WUXIANG_TECHNIQUE_IDS) {
        ...
        expect(config.effectLayers?.map((layer) => layer.id)).toEqual(['demon', 'formless']);
        expect(config.effectPlans?.map((plan) => ({
          id: plan.id,
          layers: plan.layerIds,
          consumeModeKey: plan.consumeModeKey,
        }))).toEqual([
          { id: 'formless', layers: ['demon', 'formless'], consumeModeKey: 'sect.wuxiang.form' },
          { id: 'demon', layers: ['demon'], consumeModeKey: 'sect.wuxiang.form' },
        ]);
      }
    },
  );
  ...
  it.each(
  ...
  )('$pathId/$abilityId 保持稳定 ID、目标、费用和冷却', ({ pathId, abilityId }) => {
    ...
    expect(config.targetPolicy).toEqual({ team: expected.target, scope: 'single' });
  ...
  it.each(['mirror-karma', 'demon-crossing'] as const)(
    '%s 的转相只在两种受限效果计划间选择，费用按战意条件固定',
    (pathId) => {
      const config = resolveSectAbility({
        sect: state(pathId), realm: '化神', abilityId: 'turn-form',
      }).config;
      expect(config.targetPolicy).toEqual({ team: 'self', scope: 'single' });
      expect(config.cooldown).toBe(0);
      expect(config.costs?.map((cost) => cost.mode === 'current_hp_ratio' ? cost.ratio : 0))
        .toEqual([0.04, 0.08]);
      expect(config.effectPlans?.map((plan) => ({ name: plan.name, layers: plan.layerIds })))
        .toEqual([
  ...
  it('技能详情以佛相主体、魔相追加、无相追加分段展示，不暴露内部层 ID', () => {
    const flower = resolveSectAbility({
      sect: state('mirror-karma'), realm: '化神', abilityId: 'flower-heart',
    });
    expect(flower.detailRows).toContain('效果计划「心花两忘」：佛相 + 魔相 + 无相');
    expect(flower.detailRows).toContain('效果计划「花落问罪」：佛相 + 魔相');
    expect(flower.detailRows).toContain('佛相主体：伤害：相当于60%物攻');
    expect(flower.detailRows).toContain('魔相追加：命中后：每层追加：相当于35%物攻');
    expect(flower.detailRows).toContain('无相追加：伤害：相当于30%物攻');
    expect(flower.detailRows.some((row) => row.includes('demon + formless'))).toBe(false);

    const mirrorSkandhas = resolveSectAbility({
      sect: state('mirror-karma'), realm: '化神', abilityId: 'five-skandhas',
    });
    expect(mirrorSkandhas.detailRows).toContain(
      '佛相主体：命中后：驱散：目标1个正面状态',
    );
    const demonSkandhas = resolveSectAbility({
      sect: state('demon-crossing'), realm: '化神', abilityId: 'five-skandhas',
    });
    expect(demonSkandhas.detailRows).toContain(
      '佛相主体：施展后：净化：自身1个负面状态',
    );
  });
  ...
  it('佛魔同炉只逐门修改六项明确的无相 C 字段', () => {
    const formlessEffects = (abilityId: string, nodes: string[] = []) =>
      resolveSectAbility({
        sect: state('demon-crossing', nodes), realm: '化神', abilityId,
      }).config.effectLayers?.find((layer) => layer.id === 'formless')?.effects ?? [];
    const base = Object.fromEntries(WUXIANG_TECHNIQUE_IDS.map((abilityId) => [
      abilityId,
      formlessEffects(abilityId),
    ]));
    const furnace = Object.fromEntries(WUXIANG_TECHNIQUE_IDS.map((abilityId) => [
      abilityId,
      formlessEffects(abilityId, ['demon-one-furnace']),
    ]));

    expect(base['flower-heart'][0]).toMatchObject({
      type: 'damage', params: { dynamicScalars: [{ coefficientCap: 0.4 }] },
    });
    expect(furnace['flower-heart'][0]).toMatchObject({
      type: 'damage', params: { dynamicScalars: [{ coefficientCap: 0.6 }] },
    });
    expect(base['blood-tide'][0]).toMatchObject({
      type: 'heal', params: { value: { targetMaxHpRatio: 0.05 } },
    });
    expect(furnace['blood-tide'][0]).toMatchObject({
      type: 'heal', params: { value: { targetMaxHpRatio: 0.08 } },
    });
    expect(base['three-knocks'][0]).toMatchObject({
      type: 'damage', params: { value: { coefficient: 0.65 } },
    });
    expect(furnace['three-knocks'][0]).toMatchObject({
      type: 'damage', params: { value: { coefficient: 0.85 } },
    });
    expect(base['observe-calamity'][0]).toMatchObject({
      type: 'shield', params: { value: { targetMaxHpRatio: 0.08 } },
    });
    expect(furnace['observe-calamity'][0]).toMatchObject({
      type: 'shield', params: { value: { targetMaxHpRatio: 0.12 } },
    });
    expect(base['five-skandhas'].some((effect) => effect.type === 'heal')).toBe(false);
    expect(furnace['five-skandhas']).toContainEqual(expect.objectContaining({
      type: 'heal', params: expect.objectContaining({
  ...
  it('36个节点都产生明确配置差异，且不重新引入已删除的特殊机制', () => {
    ...
    for (const path of WUXIANG_MODULE.definition.paths) {
      ...
      const compileAll = (nodes: string[]) => abilityIds.map((abilityId) =>
        resolveSectAbility({ sect: state(pathId, nodes), realm: '化神', abilityId }).config);
      const baseline = JSON.stringify(compileAll([]));
      for (const node of path.nodes) {
        const serialized = JSON.stringify(compileAll([node.id]));
        expect(serialized, `${path.name}/${node.name}必须改变战斗投影`).not.toBe(baseline);
        const signature = `${path.id}:${serialized}`;
        expect(signatures.has(signature), `${path.name}/${node.name}必须有唯一配置签名`).toBe(false);
        signatures.add(signature);
        for (const token of forbidden) {
          expect(serialized, `${path.name}/${node.name}不应包含 ${token}`).not.toContain(token);
        }
      }

      const representative = path.layers.map((layer) =>
        path.nodes.find((node) => node.layerId === layer.id)!.id);
      expect(() => compileAll(representative)).not.toThrow();

      const sameLayer = path.nodes.filter(
        (node) => node.layerId === path.layers[0].id,
      ).slice(0, 2).map((node) => node.id);
