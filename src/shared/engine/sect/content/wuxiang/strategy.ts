...
abstract class WuxiangSelectionStrategy implements AbilitySelectionStrategy {
  ...
  protected result(
    ...
    const index = new SectStrategyCandidates(WUXIANG_SECT_ID, context.candidates);
...
export class WuxiangMirrorSelectionStrategy extends WuxiangSelectionStrategy {
  select(context: AbilitySelectionContext): AbilitySelectionResult | null {
    ...
    const shouldTurn = this.tacticId === 'guard'
      ? war >= 5
    ...
    if (this.tacticId === 'guard' && karma < 3) {
      return this.result(context, ['blood-tide', 'observe-calamity', 'reed-crossing', 'flower-heart']);
    }
    return this.result(context, ['flower-heart', 'three-knocks', 'five-skandhas', 'blood-tide']);
  }
}
  ...
  select(context: AbilitySelectionContext): AbilitySelectionResult | null {
    ...
    const shouldTurn = this.tacticId === 'trial-fire'
      ...
      : this.tacticId === 'sink-boat'
        ...
        : war >= 6 || (war >= 3 && hp < 0.25);
    if (shouldTurn) return this.result(context, ['turn-form'], 800);
    return this.result(context, this.tacticId === 'sink-boat'
