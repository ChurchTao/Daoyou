...
const nodes = layers.flatMap((layer) =>
  Array.from({ length: 3 }, (_, index) => ({
    id: `fixture-${String(layer)}-${index + 1}`,
    layer,
    name: '样例节点',
    description: '样例节点行为',
  })),
);
const alternateNodes = layers.flatMap((layer) =>
  Array.from({ length: 3 }, (_, index) => ({
    id: `fixture-alternate-${String(layer)}-${index + 1}`,
    layer,
    name: '样例节点',
    description: '第二流派节点行为',
  })),
);
  ...
  paths: [
    {
      ...
      name: '样例流派',
      description: '样例',
      levelBenefitDescription: '样例成长',
      defaultTacticId: 'fixture-tactic',
      nodes,
      tactics: [
        { id: 'fixture-tactic', name: '样例战术', description: '样例' },
      ],
    },
    {
      id: 'fixture-alternate-path',
      name: '第二流派',
      description: '样例',
      levelBenefitDescription: '第二成长',
      defaultTacticId: 'fixture-alternate-tactic',
      nodes: alternateNodes,
      tactics: [
        {
          id: 'fixture-alternate-tactic',
          name: '第二战术',
          description: '样例',
        },
      ],
...
const abilityConfig = {
  slug: 'sect.fixture-sect.fixture-ability-1',
  name: '样例法术',
...
const compiledAbilities = Object.fromEntries(
...
const nodeBehaviors = Object.fromEntries(
  nodes.map((node) => [
    node.id,
    {
      contribute: (context: { activeNodeIds: ReadonlySet<string> }) => ({
        operations: [
          {
...
const alternateNodeBehaviors = Object.fromEntries(
...
const fixtureModule: SectModule = {
  ...
  paths: {
    'fixture-path': {
      ...
      compileVariants: (_context, base) => ({
        ...base,
        resources: [
          { id: 'fixture-resource', name: '样例资源', initial: 0, max: 18 },
        ],
      }),
      nodeBehaviors,
  ...
  compileBase: () => ({
    defaultAbilityId: 'fixture-ability-1',
  ...
  it('第二宗门只通过模块注册即可完成定义、流派与战斗投影', () => {
    ...
    expect(() =>
      AbilityFactory.create(
        projection.abilities[projection.defaultAbilityId].config,
      ),
    ).not.toThrow();
  });
    ...
    const invalid: SectModule = {
      ...
      paths: {
        'fixture-path': {
          ...
          nodeBehaviors: Object.fromEntries(
            Object.entries(nodeBehaviors).slice(1),
          ),
        },
      },
    };
    ...
    const invalid: SectModule = {
      ...
      paths: {
        ...
        'fixture-path': {
          ...
          nodeBehaviors: {
            ...nodeBehaviors,
            [nodes[0].id]: { contribute: () => ({ operations: [] }) },
          },
        },
      },
    };
  ...
  it('拒绝跨流派重复的节点和战术ID', () => {
    ...
    const duplicateNodeModule: SectModule = {
      ...
      paths: {
        ...
        'fixture-path': {
          ...fixtureModule.paths['fixture-path'],
          definition: duplicateNodeDefinition.paths[0],
        },
        'fixture-alternate-path': {
          ...
          nodeBehaviors: {
            ...
            [duplicateNodeDefinition.paths[0].nodes[0].id]:
              alternateNodeBehaviors[alternateNodes[0].id],
          },
        },
      },
    };
    expect(() => createSectRegistry([duplicateNodeModule])).toThrow(
      '跨流派重复节点ID',
    );
    ...
    const duplicateTacticModule: SectModule = {
      ...
      paths: {
        ...
        'fixture-path': {
          ...fixtureModule.paths['fixture-path'],
          definition: duplicateTacticDefinition.paths[0],
        },
        'fixture-alternate-path': {
          ...fixtureModule.paths['fixture-alternate-path'],
          definition: duplicateTacticDefinition.paths[1],
        },
      },
    };
    expect(() => createSectRegistry([duplicateTacticModule])).toThrow(
      '跨流派重复战术ID',
    );
  });
});
