...
const pathModule = (
  ...
  compileVariants: (
    _context: unknown,
    base: ReturnType<SectModule['compileBase']>,
  ) => ({
    ...base,
    resources: [{ id: resourceId, name: resourceName, initial: 0, max: 18 }],
  }),
  nodeBehaviors: Object.fromEntries(
    FIXTURE_SECT_DEFINITION.paths[pathIndex].nodes.map((node) => [
      node.id,
      behavior(resourceId, resourceName),
    ]),
  ),
...
export const FIXTURE_SECT_MODULE: SectModule = {
  ...
  checkAdmission: ({ playerRace }) =>
    playerRace === 'human'
      ? { allowed: true }
      : { allowed: false, reason: '种族不符' },
