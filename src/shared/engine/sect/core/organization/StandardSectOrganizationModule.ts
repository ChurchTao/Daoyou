...
} from './StandardSectCapabilityPolicy';
import {
  SECT_CRAFT_CONTEXTS,
...
const tasks: readonly SectTaskDefinition[] = [
  {
    ...
    presentation: taskPresentation('清扫山门', '清理山门步道，完成一轮宗门勤务。', 25, '开始清扫'),
    target: 1,
  },
  {
    id: 'mine_patrol',
  ...
  },
  {
    ...
    presentation: taskPresentation('宗门小比', '在演武傀儡前验证本周修行。', 40, '参加小比'),
  ...
  },
  {
    ...
    presentation: taskPresentation('长老试炼', '击败传功长老化身，取得真传资格。', 0, '挑战长老试炼'),
...
const battleScenarioDefinitions = {
  ...
  weekly_tournament: { title: '宗门小比', name: '同门演武傀儡', multiplier: 0.95 },
  weekly_bounty: {
    title: '悬赏残影战',
    name: '叛徒残影',
    multiplier: 1,
    ...
    return {
      ...
      facilityEffects: {
        cultivation_room: {
          ...
          metrics: [
            { key: 'level', label: `${this.facilityName('cultivation_room', '修炼室')}等级`, value: cultivationLevel, format: 'number' as const },
        ...
        alchemy: {
          ...
          metrics: [
            { key: 'level', label: `${this.facilityName('workshop', '丹器坊')}等级`, value: workshopLevel, format: 'number' as const },
        ...
        spirit_vein: {
          ...
          metrics: [
            { key: 'level', label: `${this.facilityName('spirit_vein', '灵脉')}等级`, value: spiritVeinLevel, format: 'number' as const },
        ...
        herb_garden: {
          ...
          metrics: [
            { key: 'level', label: `${this.facilityName('herb_garden', '药田')}等级`, value: herbGardenLevel, format: 'number' as const },
...
export interface SectOrganizationTheme {
  taskPresentation?: Partial<Record<string, Partial<SectTaskDefinition['presentation']>>>;
