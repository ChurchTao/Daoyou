import type { CultivatorSectState, SectDefinition, SectMethodModifierProjection } from './types';
...
export function projectSectMethodModifiers(
  ...
  if (!sect || sect.status !== 'active' || sect.sectId !== definition.id) return [];
  return definition.methods.flatMap((method) => {
    ...
    return [{
      ...
      level,
      modifiers: [{ ...method.modifierPerLevel, value: method.modifierPerLevel.value * level }],
    }];
  });
}
