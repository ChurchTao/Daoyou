...
describe('DefaultEnergyBudgeter', () => {
  ...
  it('应在 allocate 阶段加入多样性与语义一致性奖励', () => {
    const next = budgeter.allocate(
      ...
      { recipeId: 'skill-default', valid: true, matchedTags: [], unlockedAffixCategories: ['skill_core'], reservedEnergy: 5 },
    );
...
//   低投入  → skill_variant 未解锁，2 词缀槽位
//   中投入  → skill_variant 解锁，3 词缀槽位
//   高投入  → skill_rare 解锁，4 词缀槽位
//   顶级投入 → 完整 5 词缀槽位
// ─────────────────────────────────────────────────────────────────────────────
import { resolveAffixSlotCount, CREATION_AFFIX_UNLOCK_THRESHOLDS } from '@shared/engine/creation-v2/config/CreationBalance';
...
describe('能量阈值与槽位梯次基准校准', () => {
  ...
  it('顶级投入：skill_rare 阈值对应 5 词缀槽位', () => {
    const topEnergy = CREATION_AFFIX_UNLOCK_THRESHOLDS.skill_rare + 20;
    expect(resolveAffixSlotCount(topEnergy)).toBe(5);
    expect(topEnergy).toBeGreaterThanOrEqual(CREATION_AFFIX_UNLOCK_THRESHOLDS.skill_rare);
  });

  it('阈值有序性：unlock thresholds 应严格递增', () => {
    const t = CREATION_AFFIX_UNLOCK_THRESHOLDS;
    expect(t.skill_core).toBeLessThan(t.skill_variant);
    expect(t.skill_variant).toBeLessThan(t.skill_rare);
  });
