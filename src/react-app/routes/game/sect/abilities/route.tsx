...
export default function SectAbilitiesPage() {
  ...
  useEffect(() => {
    ...
    void fetchSectCurrent().then((next) => {
      if (!cancelled) { setData(next); setDraftSlots(createAbilitySlots(next.sect?.abilityLoadout ?? [])); }
    }).catch((reason) => { if (!cancelled) setError(reason instanceof Error ? reason.message : '宗门神通读取失败'); });
  ...
  const details = useMemo(() => sect && definition
    ? definition.abilities.map((ability) => resolveSectAbility({ abilityId: ability.id, sect, realm }))
    : [], [definition, realm, sect]);
  const activeDetails = details.filter((detail) => definition?.abilities.find((ability) => ability.id === detail.id)?.occupiesActiveSlot);
  const defaultDetail = details.find((detail) => !definition?.abilities.find((ability) => ability.id === detail.id)?.occupiesActiveSlot);
  ...
  const path = definition?.paths.find((entry) => entry.id === sect?.activePathId);
  const pathState = sect?.paths.find((entry) => entry.pathId === sect.activePathId);

  const save = async () => {
    if (!sect || !changed) return;
    setBusy(true);
    try {
      const result = await mutate<{ sect: CultivatorSectState }>(fetch('/api/sects/current/ability-loadout', json('PUT', { abilityIds: draftSlots })));
    ...
    } catch (reason) { pushToast({ message: reason instanceof Error ? reason.message : '神通栏保存失败', tone: 'danger' }); }
    finally { setBusy(false); }
  };

  const toggle = (abilityId: string) => {
    if (!sect || busy) return;
    if (selected.includes(abilityId)) { setDraftSlots(createAbilitySlots(selected.filter((id) => id !== abilityId))); return; }
    if (selected.length >= 4) { pushToast({ message: '最多选择四门宗门神通', tone: 'danger' }); return; }
  ...
  const setTactic = async (tacticId: string) => {
    ...
    setBusy(true);
    try {
      const result = await mutate<{ sect: CultivatorSectState }>(fetch(`/api/sects/current/paths/${sect.activePathId}/tactic`, json('PUT', { tacticId })));
      setData((current) => current ? { ...current, sect: result.sect } : current);
      pushToast({ message: '自动战术已切换', tone: 'success' });
    } catch (reason) { pushToast({ message: reason instanceof Error ? reason.message : '战术切换失败', tone: 'danger' }); }
    finally { setBusy(false); }
  };
  ...
  return (
    <GameSceneFrame
      title="【宗门神通】"
      description={definition ? `查阅并配置${definition.name}的基础神通与当前流派变体。` : '拜入宗门后，可在此配置宗门神通。'}
      headerMeta={error ? <GameSceneNote tone="danger">{error}</GameSceneNote> : undefined}
      aside={definition ? <div className="space-y-2 text-sm leading-7"><p>宗门：{definition.name}</p><p>流派：{path?.name ?? '尚未激活'}</p><p>战术：{path?.tactics.find((entry) => entry.id === pathState?.tacticId)?.name ?? '默认'}</p></div> : undefined}
    >
      {!sect || !definition ? <InkNotice>尚未拜入宗门。</InkNotice> : <>
        {defaultDetail ? <InkCard><strong>{defaultDetail.name}</strong><span className="text-ink-secondary ml-2 text-sm">默认神通 · 不占主动栏</span><SectAbilityDetails detail={defaultDetail} /></InkCard> : null}
        <InkCard className="mt-3">
          <h3 className="text-lg font-semibold">四个主动栏</h3>
          <p className="text-ink-secondary mt-1 text-sm">从神通卷中快捷选择至多四门，确认后统一保存配置。</p>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <InkButton disabled={busy || selected.length === 0} onClick={() => setDraftSlots(EMPTY_SLOTS)}>
              清空全部
            </InkButton>
            <InkButton disabled={busy} onClick={() => setPickerOpen(true)}>选择神通</InkButton>
            <InkButton variant="primary" disabled={busy || !changed} onClick={() => void save()}>
              {busy ? '保存中' : '保存配置'}
            </InkButton>
            {changed ? <span className="text-crimson text-sm">配置尚未保存</span> : null}
          </div>
        </InkCard>
        {path && pathState ? <InkCard className="mt-4"><h3 className="font-semibold">自动战术</h3><div className="mt-2 grid gap-2 md:grid-cols-3">{path.tactics.map((tactic) => <button type="button" key={tactic.id} disabled={busy} onClick={() => void setTactic(tactic.id)} className={`p-3 text-left text-sm leading-6 ${pathState.tacticId === tactic.id ? 'bg-crimson/10 text-crimson' : 'bg-ink/5'}`}><strong>{tactic.name}</strong><br />{tactic.description}</button>)}</div></InkCard> : null}
        ...
        <InkModal
          ...
          footer={(
            <div className="flex items-center justify-between gap-3">
              <span className="text-ink-secondary text-sm">已选 {selected.length} / 4</span>
