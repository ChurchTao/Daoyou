...
function SectHallBody() {
  ...
  if (error)
    return (
      <SectQueryError
        error={error}
        retry={() =>
          void (current.error ? current.retry() : membersQuery.retry())
        }
      />
    );
  if (!current.data || !membersQuery.data)
    return <SectPageLoading sceneKey="hall" />;
  ...
  if (!sect)
    return (
      <SectScene sceneKey="hall" descriptionOverride="正式入宗后方可进入。">
        <InkNotice>尚未拜入宗门。</InkNotice>
      </SectScene>
    );
  if (!overview)
    return (
      <SectScene sceneKey="hall" descriptionOverride="身份记录尚未归档。">
        <InkNotice>宗门概览暂不可用。</InkNotice>
      </SectScene>
    );
    ...
    <SectScene
      ...
      aside={
        <div className="space-y-2 text-sm leading-7">
          <p>当前身份：{rankLabel(rank)}</p>
          <p>宗门贡献：{sect.contribution}</p>
          <p>心法上限：{overview.methodLevelCap}级</p>
        </div>
      }
    >
      <div className="relative grid gap-3 border-y border-amber-950/10 py-5 md:grid-cols-3">
        ...
        <InkCard>
          ...
          <strong className="mt-2 block text-xl">
            {overview.stipend.spiritStones.toLocaleString()} 灵石
          </strong>
            ...
            <>
              <strong className="mt-2 block text-xl">
                {getSectFacilityLabel(
                  data.definition!.id,
                  overview.project.facilityKey,
                )}{' '}
                · {overview.project.targetLevel}级
              </strong>
              <p className="mt-2 text-sm">
                {overview.project.progress.toLocaleString()} /{' '}
                {overview.project.target.toLocaleString()} 建设点
              </p>
              <div className="bg-ink/10 mt-3 h-1.5 overflow-hidden">
                <div
                  className="bg-crimson h-full"
                  style={{
                    width: `${Math.min(100, (overview.project.progress / overview.project.target) * 100)}%`,
                  }}
                />
              </div>
            </>
      ...
      <section className="bg-[rgba(255,250,234,0.46)] px-4 py-5 shadow-[inset_16px_0_24px_rgba(91,61,25,0.05)]">
        <div className="mb-3 flex items-end justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold">同门名录</h2>
            <p className="text-ink-secondary mt-1 text-sm">
              共 {members.total} 位正式成员，职务首版仅作展示。
            </p>
          </div>
        </div>
            ...
            <tbody>
              {members.items.map((member) => (
                ...
                >
                  ...
                  <td className="p-2">
                    {SECT_RANK_LABELS[member.discipleRank]}
                  </td>
                  <td className="p-2">
