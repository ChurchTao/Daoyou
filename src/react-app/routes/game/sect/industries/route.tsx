...
function SectIndustriesBody() {
  ...
  return (
    <SectScene
      ...
      aside={
        <div className="space-y-2 text-sm leading-7">
          <p>
            今日建设贡献：{data.donatedContributionToday} /{' '}
            {data.dailyContributionCap}
          </p>
          <p>
            剩余额度：
            {Math.max(
    ...
    >
      <section className="bg-slate-800/5 [background-image:linear-gradient(rgba(61,74,73,.06)_1px,transparent_1px),linear-gradient(90deg,rgba(61,74,73,.06)_1px,transparent_1px)] [background-size:24px_24px] px-4 py-5">
          ...
          {data.project ? (
            <>
              <div className="flex items-start justify-between gap-3">
                <strong>
                  {facilityLabel(data.project.facilityKey)}升至{' '}
                  {data.project.targetLevel} 级
                </strong>
                <span className="text-crimson text-sm">
                  {Math.floor(
                    (data.project.progress / data.project.target) * 100,
                  )}
                  %
                </span>
              </div>
              <p className="mt-2 text-sm">
                {data.project.progress.toLocaleString()} /{' '}
                {data.project.target.toLocaleString()} 建设点
              </p>
              <div className="bg-ink/10 mt-3 h-2">
                <div
                  className="bg-crimson h-full transition-[width]"
                  style={{
                    width: `${Math.min(100, (data.project.progress / data.project.target) * 100)}%`,
                  }}
                />
              </div>
            </>
            ...
            <>
              <strong>长老工程待启</strong>
              <p className="text-ink-secondary mt-2 text-sm">
                提交首笔有效建设捐献时，将按本周优先级原子开启工程。
              </p>
            </>
          )}
        </InkCard>
      ...
      <section className="border-y border-slate-900/10 py-5">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {data.facilities.map((facility) => (
            <InkCard key={facility.key} padding="sm" className="mb-0">
              <div className="flex items-center justify-between">
                <strong>{facilityLabel(facility.key)}</strong>
                <span className="text-crimson">
                  {lockedFacilities.includes(facility.key)
                    ? '锁定'
                    : `${facility.level}级`}
                </span>
              </div>
        ...
        {data.recentActivity.length ? (
          <ul className="mt-3 space-y-2 text-sm leading-6">
            {data.recentActivity.map((item) => (
              <li
                key={item.id}
                className="border-ink/15 border-b border-dashed pb-2"
              >
                <strong>{item.memberName}</strong> 完成一笔「{item.demandId}
                」捐献，宗门建设 +{item.constructionPoints}，个人贡献 +
                {item.contribution}
                <time className="text-ink-secondary ml-2 text-xs">
          ...
          ? products.artifacts
              .filter(
                (item): item is typeof item & { id: string } =>
                  Boolean(item.id) && !item.isEquipped,
              )
              .map((item) => ({
                id: item.id,
  ...
  return (
    <InkCard>
      ...
      <p className="mt-2 text-xs">
        同时增加 {demand.constructionPoints} 建设点
      </p>
      {needsItem ? (
        <InkSelect
          className="mt-3"
      ...
      <InkButton
        variant="primary"
        disabled={
          busy || capRemaining < demand.contribution || (needsItem && !itemId)
        }
        onClick={() =>
          void run(
      ...
      >
        {capRemaining < demand.contribution ? '今日额度不足' : '捐献一份'}
      </InkButton>
    </InkCard>
  );
}
