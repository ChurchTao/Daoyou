...
export function SectQueryProvider({ children }: { children: ReactNode }) {
  const [resources, setResources] = useState<
    Record<string, SectResourceSnapshot>
  >({});
  ...
  return (
    <SectQueryContext.Provider value={value}>
      {children}
    </SectQueryContext.Provider>
  );
}
...
export function useSectResourceQuery<T>(
  ...
  if (!context)
    throw new Error('useSectResourceQuery 必须在 SectQueryProvider 中使用');
  const snapshot = context.resources[key] as
    SectResourceSnapshot<T> | undefined;
...
export function useSectPresentation(): ResolvedSectPresentation {
  const { data } = useSectCurrentQuery();
  const sectId = data?.definition?.id;
  return useMemo(
    () =>
      sectId
        ? getSectPresentation(sectId)
