...
import { SectError, SectService, type SectServiceInstance } from '@server/lib/services/SectService';
...
export function createSectsRouter(dependencies: {
...
router.get('/catalog', requireActiveCultivator(), async (c) => {
  ...
  return c.json({ success: true, data: {
    ...
    sects: sectService.listAvailableDefinitions({
...
router.get('/current', requireActiveCultivator(), async (c) => {
  ...
  const definition = sect ? runtime.registry.require(sect.sectId).definition : null;
...
router.get('/:sectId', requireActiveCultivator(), async (c) => {
  ...
  if (!cultivator?.id) return c.json({ success: false, error: '当前没有活跃角色' }, 404);
  try {
    const definition = runtime.registry.get(c.req.param('sectId'))?.definition;
...
router.post('/:sectId/trial', requireActiveCultivator(), async (c) => {
  ...
  try {
    ...
    const { trainee, opponent } = sectService.createTrialScenario(module.definition.id, runtimeCultivator);
