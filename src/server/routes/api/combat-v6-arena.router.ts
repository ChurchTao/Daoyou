...
router.get('/:battleId', async (c) => {
  ...
  const { runtime, participant } = await ownedArenaV6(
    id,
...
router.get(
  ...
  async (c, next) => {
    ...
    const id = z.uuid().parse(c.req.param('battleId'));
    await ownedArenaV6(id, c.get('activeCultivatorRef')!);
    return next();
  },
  upgradeWebSocket((c) => {
    ...
    return {
      async onOpen(_event, ws) {
        try {
          ...
          const owned = await ownedArenaV6(id, actor);
          viewerId = owned.participant.unitId;
          dispose = await subscribeArenaV6(id, () => {
            chain = chain
              .then(async () => {
                if (closed) return;
                const { runtime } = await ownedArenaV6(id, actor);
                if (runtime.revision <= lastRevision) return;
                lastRevision = runtime.revision;
                ws.send(JSON.stringify(arenaSocketState(runtime, viewerId!)));
              })
              .catch(() => {
          ...
          await store.touch(`${id}:${viewerId}`, connection);
          if (closed) {
