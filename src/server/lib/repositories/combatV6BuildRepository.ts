...
export async function findCombatV6Profile(membershipId: string, q: DbExecutor) {
  const [row] = await q
    .select()
...
export async function listCombatV6MethodLevels(
  ...
  const rows = await q
    .select({
      methodId: combatV6MethodProgress.methodId,
      level: combatV6MethodProgress.level,
    })
...
export async function loadActiveCombatV6Build(
  ...
  if (!profile || profile.status !== 'active' || !profile.activePathId)
    return null;
  ...
  const loadouts = await q
    .select({
      id: combatV6MeridianLoadouts.id,
      pathId: combatV6MeridianLoadouts.pathId,
      revision: combatV6MeridianLoadouts.revision,
    })
  ...
  for (const loadout of loadouts) {
    const nodes = await q
      .select({
        nodeId: combatV6MeridianNodes.nodeId,
        layer: combatV6MeridianNodes.layer,
      })
  ...
  const slots = await q
    .select({
      slot: combatV6ManualSlots.slot,
      manualId: combatV6ManualSlots.manualId,
    })
  ...
  const equipmentRows = await q
    ...
    .innerJoin(
      inventoryItems,
      eq(combatV6EquipmentLoadouts.equipmentInstanceId, inventoryItems.id),
    )
  ...
  return {
    ...
    sect: {
      ...
      methods,
      meridianDepth:
        profile.meridianDepth as SectCombatProgressV6['meridianDepth'],
