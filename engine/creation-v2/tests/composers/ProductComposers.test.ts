...
describe('SkillBlueprintComposer', () => {
  it('技能 golden case: fire ore → active_skill with damage effect', () => {
    ...
    expect(outcome!.outcomeKind).toBe('active_skill');
    expect(outcome!.productModel.productType).toBe('skill');
    expect(outcome!.abilityConfig.type).toBe(AbilityType.ACTIVE_SKILL);
    expect(outcome!.abilityConfig.slug).toBe(outcome!.productModel.slug);
    expect(outcome!.abilityConfig.effects?.length).toBeGreaterThan(0);
    // 第一个效果应是 damage 或 apply_buff 类型
    const firstEffect = outcome!.abilityConfig.effects![0];
  ...
  it('heal 材料 → active_skill 带 heal 效果', () => {
    ...
    expect(outcome).not.toBeNull();
    const effects = outcome!.abilityConfig.effects ?? [];
  ...
  it('技能蓝图有合法的 mpCost 和 cooldown', () => {
    ...
    const cfg = outcome.abilityConfig;
...
describe('ArtifactBlueprintComposer', () => {
  it('法宝 golden case → artifact outcome with passive listeners', () => {
    ...
    expect(outcome).not.toBeNull();
    expect(outcome!.outcomeKind).toBe('artifact');
    expect(outcome!.productModel.productType).toBe('artifact');
    expect(outcome!.abilityConfig.type).toBe(AbilityType.PASSIVE_SKILL);
    expect(outcome!.abilityConfig.listeners?.length).toBeGreaterThan(0);
    if (outcome!.productModel.productType === 'artifact') {
      expect(outcome!.productModel.progressionPolicy).toBe('reforgeable');
    }
  });
  ...
  it('法宝 listener 每个至少含一个 effect', () => {
    ...
    if (!outcome) return;
    for (const listener of outcome.abilityConfig.listeners ?? []) {
...
describe('GongFaBlueprintComposer', () => {
  it('功法 golden case → gongfa outcome with attribute_stat_buff listener', () => {
    ...
    expect(outcome).not.toBeNull();
    expect(outcome!.outcomeKind).toBe('gongfa');
    expect(outcome!.productModel.productType).toBe('gongfa');
    expect(outcome!.abilityConfig.type).toBe(AbilityType.PASSIVE_SKILL);
    const listeners = outcome!.abilityConfig.listeners ?? [];
    ...
    if (outcome!.productModel.productType === 'gongfa') {
      expect(outcome!.productModel.progressionPolicy).toBe('comprehension');
    }
  });
...
describe('CreationOrchestrator with ProductComposerRegistry', () => {
  it('skill → active_skill, artifact → artifact, gongfa → gongfa', () => {
    ...
    if (skillOutcome) expect(skillOutcome.outcomeKind).toBe('active_skill');
    if (artifactOutcome) expect(artifactOutcome.outcomeKind).toBe('artifact');
    if (gongfaOutcome) expect(gongfaOutcome.outcomeKind).toBe('gongfa');
  });
});
