...
- [route.ts](file://app/api/mail/route.ts)
- [read/route.ts](file://app/api/mail/[id]/read/route.ts)
- [claim/route.ts](file://app/api/mail/[id]/claim/route.ts)
- [unread-count/route.ts](file://app/api/mail/unread-count/route.ts)
- [MailList.tsx](file://components/mail/MailList.tsx)
- [MailDetailModal.tsx](file://components/mail/MailDetailModal.tsx)
- [page.tsx](file://app/mail/page.tsx)
...
## 简介
本系统为修仙游戏《万界道友》中的邮件系统，提供传音玉简功能，用于向修士发送系统通知、战斗奖励等信息。系统支持邮件的创建、状态变更（已读/未读）、奖励领取及未读计数展示。前端通过`MailList`和`MailDetailModal`组件实现邮件列表与详情交互，后端通过`/api/mail/*`路由处理REST请求，业务逻辑由`MailService`统一管理。数据存储于PostgreSQL数据库中，使用Drizzle ORM进行操作。
...
subgraph "API 路由"
A[/api/mail/route.ts]
B[/api/mail/[id]/read/route.ts]
C[/api/mail/[id]/claim/route.ts]
D[/api/mail/unread-count/route.ts]
end
subgraph "前端组件"
E[MailList.tsx]
...
**Section sources**
- [app/api/mail/](file://app/api/mail/)
...
participant API as API路由 (/api/mail/*)
...
服务-->>API : 返回邮件数据
API-->>前端 : 返回JSON数据
前端->>API : POST /api/mail/{id}/read
API->>服务 : 更新邮件为已读
服务->>数据库 : 更新isRead字段
数据库-->>服务 : 确认更新
服务-->>API : 返回成功
API-->>前端 : 返回成功
前端->>API : POST /api/mail/{id}/claim
...
```mermaid
flowchart TD
A[GET /api/mail] --> B{验证用户}
B --> |未登录| C[返回401]
B --> |已登录| D[获取活跃角色]
D --> |无角色| E[返回404]
D --> |有角色| F[查询角色邮件]
F --> G[返回邮件列表]
H[POST /api/mail/{id}/read] --> I{验证用户}
I --> |未登录| J[返回401]
I --> |已登录| K[验证邮件归属]
K --> |无权访问| L[返回403]
K --> |有权访问| M[更新isRead为true]
M --> N[返回成功]
O[POST /api/mail/{id}/claim] --> P{验证用户}
P --> |未登录| Q[返回401]
P --> |已登录| R[验证邮件归属]
R --> |无权访问| S[返回403]
R --> |有权访问| T{已领取?}
