# 战斗系统

<cite>
**本文档引用文件**   
- [battleEngine.ts](file://engine/battleEngine.ts)
- [route.ts](file://app/api/battle/route.ts)
- [BattlePageLayout.tsx](file://components/BattlePageLayout.tsx)
- [BattleReplayViewer.tsx](file://components/BattleReplayViewer.tsx)
- [page.tsx](file://app/battle/page.tsx)
- [challenge-battle/route.ts](file://app/api/rankings/challenge-battle/route.ts)
- [challenge/page.tsx](file://app/battle/challenge/page.tsx)
- [rankings.ts](file://lib/redis/rankings.ts)
- [schema.ts](file://lib/drizzle/schema.ts)
- [cultivator.ts](file://types/cultivator.ts)
</cite>

## 目录
1. [简介](#简介)
2. [战斗系统架构](#战斗系统架构)
3. [核心战斗逻辑](#核心战斗逻辑)
4. [挑战战斗与排行榜](#挑战战斗与排行榜)
5. [战斗数据结构](#战斗数据结构)
6. [API请求流程](#api请求流程)
7. [前端组件与渲染](#前端组件与渲染)
8. [战斗事件流时序图](#战斗事件流时序图)
9. [性能优化建议](#性能优化建议)
10. [结论](#结论)

## 简介
本系统实现了完整的战斗机制，涵盖普通战斗、挑战战斗、战报回放与历史记录功能。核心逻辑由`battleEngine.ts`驱动，通过API接口触发战斗并生成流式战报，前端组件负责渲染战斗过程与结果。挑战战斗与排行榜系统深度联动，实现排名更新与奖励机制。

## 战斗系统架构
系统采用分层架构，后端战斗引擎负责计算，API层处理请求，前端组件负责展示。战斗结果持久化存储于数据库，支持历史回放。

```mermaid
graph TB
subgraph "前端"
A[BattlePageLayout]
B[BattleReplayViewer]
C[BattleTimelineViewer]
end
subgraph "API层"
D[/api/battle/route.ts]
E[/api/rankings/challenge-battle/route.ts]
end
subgraph "后端"
F[battleEngine.ts]
G[rankings.ts]
H[schema.ts]
end
A --> D
B --> D
C --> D
D --> F
E --> F
F --> H
G --> H
```

**Diagram sources**
- [battleEngine.ts](file://engine/battleEngine.ts)
- [route.ts](file://app/api/battle/route.ts)
- [BattlePageLayout.tsx](file://components/BattlePageLayout.tsx)
- [BattleReplayViewer.tsx](file://components/BattleReplayViewer.tsx)
- [schema.ts](file://lib/drizzle/schema.ts)

## 核心战斗逻辑
战斗引擎实现了完整的战斗计算模型，包括伤害公式、状态效果、胜负判定等。

### 战斗计算模型
战斗计算基于角色属性、技能、装备等多维度数据，通过一系列公式计算伤害与效果。

```mermaid
flowchart TD
Start([开始战斗]) --> Init["初始化战斗单位"]
Init --> Loop["战斗循环"]
Loop --> Check["检查胜负"]
Check --> |未分胜负| Turn["回合开始"]
Turn --> Status["状态效果结算"]
Status --> Speed["速度判定"]
Speed --> Action["执行技能"]
Action --> Cooldown["冷却递减"]
Cooldown --> Loop
Check --> |分出胜负| End["结束战斗"]
```

**Diagram sources**
- [battleEngine.ts](file://engine/battleEngine.ts)

### 伤害公式
伤害计算综合考虑攻击力、防御力、元素克制、暴击等因素。

```mermaid
flowchart TD
Base["基础伤害 = 技能威力 * (1 + 灵力/150)"] --> Root["灵根加成"]
Root --> Artifact["法宝伤害加成"]
Artifact --> Crit["暴击判定"]
Crit --> Defense["防御减免"]
Defense --> Final["最终伤害"]
```

**Diagram sources**
- [battleEngine.ts](file://engine/battleEngine.ts)

### 胜负判定机制
当任一方气血值降至0或战斗回合达到上限时，判定胜负。

**Section sources**
- [battleEngine.ts](file://engine/battleEngine.ts#L715-L731)

## 挑战战斗与排行榜
挑战战斗系统与排行榜深度集成，实现排名更新、挑战次数限制、保护机制等功能。

### 匹配机制
挑战者可选择排名高于自己的对手进行挑战，系统验证挑战条件后执行战斗。

**Section sources**
- [challenge-battle/route.ts](file://app/api/rankings/challenge-battle/route.ts#L125-L128)

### 排行榜联动逻辑
挑战成功后，挑战者与被挑战者排名互换，系统更新排行榜并记录战斗结果。

```mermaid
sequenceDiagram
participant 前端
participant API
participant 排行榜服务
participant 数据库
前端->>API : 发起挑战请求
API->>排行榜服务 : 验证挑战条件
排行榜服务-->>API : 返回验证结果
API->>战斗引擎 : 执行战斗
战斗引擎-->>API : 返回战斗结果
API->>排行榜服务 : 更新排名
排行榜服务-->>API : 返回更新结果
API->>数据库 : 记录战斗结果
数据库-->>API : 返回记录结果
API-->>前端 : 返回完整结果
```

**Diagram sources**
- [challenge-battle/route.ts](file://app/api/rankings/challenge-battle/route.ts)
- [rankings.ts](file://lib/redis/rankings.ts)
- [schema.ts](file://lib/drizzle/schema.ts)

## 战斗数据结构
系统定义了清晰的数据结构，用于表示战斗相关实体。

### 核心数据结构
```mermaid
classDiagram
class BattleEngineResult {
+winner : Cultivator
+loser : Cultivator
+log : string[]
+turns : number
+playerHp : number
+opponentHp : number
+timeline : TurnSnapshot[]
}
class TurnSnapshot {
+turn : number
+player : TurnUnitSnapshot
+opponent : TurnUnitSnapshot
}
class TurnUnitSnapshot {
+hp : number
+mp : number
+statuses : StatusEffect[]
}
class Cultivator {
+id : string
+name : string
+realm : string
+attributes : Attributes
+skills : Skill[]
+inventory : Inventory
+equipped : EquippedItems
}
BattleEngineResult --> TurnSnapshot
TurnSnapshot --> TurnUnitSnapshot
TurnSnapshot --> TurnUnitSnapshot
```

**Diagram sources**
- [battleEngine.ts](file://engine/battleEngine.ts)
- [cultivator.ts](file://types/cultivator.ts)

## API请求流程
系统提供RESTful API接口，支持触发战斗与获取战斗结果。

### 普通战斗API流程
```mermaid
sequenceDiagram
participant 前端
participant API
participant 战斗引擎
participant 数据库
前端->>API : POST /api/battle
API->>API : 验证用户身份
API->>API : 获取角色数据
API->>战斗引擎 : 执行战斗
战斗引擎-->>API : 返回战斗结果
API->>API : 生成战斗播报
API->>数据库 : 记录战斗结果
数据库-->>API : 返回记录结果
API-->>前端 : SSE流式返回结果
```

**Diagram sources**
- [route.ts](file://app/api/battle/route.ts)

### 挑战战斗API流程
```mermaid
sequenceDiagram
participant 前端
participant API
participant 战斗引擎
participant 排行榜服务
participant 数据库
前端->>API : POST /api/rankings/challenge-battle
API->>API : 验证用户身份
API->>API : 检查挑战次数
API->>API : 获取角色数据
API->>战斗引擎 : 执行战斗
战斗引擎-->>API : 返回战斗结果
API->>API : 生成战斗播报
API->>排行榜服务 : 更新排名
排行榜服务-->>API : 返回更新结果
API->>数据库 : 记录战斗结果
数据库-->>API : 返回记录结果
API-->>前端 : SSE流式返回结果
```

**Diagram sources**
- [challenge-battle/route.ts](file://app/api/rankings/challenge-battle/route.ts)

## 前端组件与渲染
前端组件负责展示战斗过程与结果，提供良好的用户体验。

### BattlePageLayout组件
作为战斗页面的布局容器，提供统一的页面结构和操作按钮。

**Section sources**
- [BattlePageLayout.tsx](file://components/BattlePageLayout.tsx)

### BattleReplayViewer组件
展示战斗回放，包括数值变化、状态效果和战报文本。

**Section sources**
- [BattleReplayViewer.tsx](file://components/BattleReplayViewer.tsx)

### BattleTimelineViewer组件
可视化展示战斗时间线，支持回合回放与控制。

**Section sources**
- [BattleTimelineViewer.tsx](file://components/BattleTimelineViewer.tsx)

## 战斗事件流时序图
完整展示战斗从触发到结束的事件流。

```mermaid
sequenceDiagram
participant 用户
participant 前端
participant API
participant 战斗引擎
participant 排行榜服务
participant 数据库
用户->>前端 : 发起战斗请求
前端->>API : 调用战斗API
API->>API : 验证身份与参数
API->>API : 获取角色数据
API->>战斗引擎 : 执行战斗模拟
战斗引擎-->>API : 返回战斗结果
API->>API : 生成战报文本
API->>排行榜服务 : 更新排名(挑战战斗)
排行榜服务-->>API : 返回排名更新
API->>数据库 : 存储战斗记录
数据库-->>API : 返回存储结果
API-->>前端 : 流式返回战斗数据
前端->>用户 : 渲染战斗过程
```

**Diagram sources**
- [route.ts](file://app/api/battle/route.ts)
- [challenge-battle/route.ts](file://app/api/rankings/challenge-battle/route.ts)
- [battleEngine.ts](file://engine/battleEngine.ts)

## 性能优化建议
为提升系统性能与用户体验，提出以下优化建议。

### 战斗日志压缩
对战斗日志进行压缩存储，减少数据库占用空间。

**Section sources**
- [route.ts](file://app/api/battle/route.ts#L115-L121)

### 前端渲染优化
采用虚拟滚动技术优化长列表渲染，减少DOM操作。

**Section sources**
- [BattleTimelineViewer.tsx](file://components/BattleTimelineViewer.tsx)

### 缓存策略
对排行榜数据进行缓存，减少数据库查询压力。

**Section sources**
- [rankings.ts](file://lib/redis/rankings.ts)

## 结论
本战斗系统实现了完整的战斗功能，包括普通战斗、挑战战斗、战报回放与历史记录。系统架构清晰，逻辑严谨，性能优良，为用户提供流畅的战斗体验。