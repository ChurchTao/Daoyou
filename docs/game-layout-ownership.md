...
- 全局地图、野外与坊市选址入口统一进入 `/game/map-v2`；旧 `/game/map` 仅作为兼容地址，通过 replace 重定向保留查询参数，不再开放旧版地图入口。
- `/game/map-v2` 是 Phaser 沉浸地图，接入世界总览与天南、慕兰草原、乱星海、大晋皇朝、南疆、北境冰原。地图路由拥有自身顶部控件，`MapSceneChrome` 对此路由不再重复绘制：左上角为“关闭地图 / 返回上层”，右上角采用紧凑单行工具条，合并区域名、直接可输入的地点搜索框与类型下拉，搜索结果和类型选项在下方就地展开。关闭固定返回洞府，上层返回人界总览；只保留一个非模态节点信息与操作面板，画布负责地形、镜头与标记。`types` 保存筛选，玩法历史项的 `mapReturnTo` 保留直接返回来源。
