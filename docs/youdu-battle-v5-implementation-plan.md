...
>
> 本文只确定实现边界与 V1 降级方案，不代表幽都已经进入生产实现。
>
> 分析基线：2026-07-22 当前工作区。当前工作区中的 `controlHitBonus`、`statusVisibility` 与 `stackPriority` 等天衍相关改动按可用能力计入，但不得覆盖或拆改其未提交实现。
>
...
| 持续时间 | Buff 在宿主每次行动后减 1 | 与“按目标自身行动计 3 回合”一致 |
...
V1 仅《照影》本身使用必然命中。照影将目标 `EVASION_RATE` 以 OVERRIDE 置 0，但其他能力仍遵守全局最低 3%闪避。镇魄司命不再额外改写后续技能命中，采用第 3.4 节的近似增伤。
...
- 运行时 duration 配置为 2：它通常在目标的 `ActionPostEvent` 中创建，而引擎会在同一帧随后立刻执行一次 duration tick；配置 2 才会在该次结算后留下 1 回合。控制抵抗或免疫分支也统一使用同一配置，保证快慢手顺序下至少覆盖幽都下一次加层机会。
...
- 技能 completion effect 在命中后 `consume_all`。
