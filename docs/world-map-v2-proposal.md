...
日期：2026-09-19。状态：供讨论的设计方案，尚未实施。

素材进度：世界总览与天南区域底画已选定并导出到 `public/assets/maps/`，见[素材交付清单](game-map-art.md)。天南采用独立区域构图；旧预览及中间稿已清理，Phaser 地图功能尚未接入。美术规范统一维护在[国画水墨地图 skill](../.agents/skills/daoyou-map-art/SKILL.md)。
...
一个地图页面生命周期只创建一个 `Phaser.Game`，持有总览与区域两个 Scene 类型，区域 Scene 根据配置加载不同区域。React 只在选区、选点等离散事件更新状态，不逐帧同步相机坐标。
