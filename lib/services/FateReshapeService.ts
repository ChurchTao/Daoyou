...
async function readSession(
  ...
  const session = await redis.get<FateReshapeSessionStore>(
...
async function restoreSession(session: FateReshapeSessionStore): Promise<void> {
  ...
  await redis.set(buildSessionKey(session.cultivatorId), session, {
    ex: getRemainingTtlSeconds(session.expiresAt),
  });
}
...
export const FateReshapeService = {
    ...
    return withCultivatorLock(cultivatorId, async () => {
      ...
      try {
        await getExecutor().transaction(async (tx) => {
          ...
          await redis.set(buildSessionKey(cultivatorId), session, {
            ex: FATE_RESHAPE_SESSION_TTL_SEC,
          });
        });
      } catch (error) {
        await redis.del(buildSessionKey(cultivatorId));
  ...
  async rerollSession(cultivatorId: string): Promise<FateReshapeSessionDTO> {
    return withCultivatorLock(cultivatorId, async () => {
      ...
      await redis.set(buildSessionKey(cultivatorId), nextSession, {
        ex: getRemainingTtlSeconds(session.expiresAt),
      });
